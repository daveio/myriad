...
export function parseByteSizeToBytes(
  ...
  const sizeMatch = normalisedValue.match(
    /(\d+(?:\.\d+)?)\s*(b|kb|mb|gb|tb)\b/i,
  );
...
export function parseContextToTokens(
  value: string | null | undefined,
): number | null {
  const normalisedValue = value?.trim();
...
export function parsePullCount(
  value: string | null | undefined,
): number | null {
  const normalisedValue = value?.trim();
...
export function parseParameterEstimate(
  value: string | null | undefined,
): number | null {
  ...
  const mixtureMatch = normalisedValue.match(
    /(\d+(?:\.\d+)?)\s*x\s*(\d+(?:\.\d+)?)\s*b\b/,
  );
  ...
  const scalarMatches = [
    ...normalisedValue.matchAll(/(\d+(?:\.\d+)?)\s*([mb])\b/g),
  ];
