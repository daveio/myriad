...
import type { OllamaCataloguePayload, OllamaModel, OllamaTag } from '../../types/ollama'
...
export async function fetchOllamaHtml(url: string): Promise<string> {
  ...
  if (!response.ok) {
    throw new Error(`Ollama request failed for ${url}: ${response.status} ${response.statusText}`)
  }
...
export function parseLibraryModels(html: string): OllamaModel[] {
  ...
  document('li[x-test-model]').each((_rowIndex, element) => {
    ...
    const name = cleanText(titleElement.attr('title') ?? titleElement.find('span').first().text() ?? slug)

    if (!name || !slug) {
      return
    }

    const pullCountLabel = cleanText(row.find('[x-test-pull-count]').first().text()) || null
    const tagCountText = cleanText(row.find('[x-test-tag-count]').first().text())
    const updatedElement = row.find('[x-test-updated]').first()
    const updatedAtTitle = updatedElement.closest('[title]').attr('title') ?? null

    models.push({
      capabilities: uniqueTexts(row.find('[x-test-capability]').map((_chipIndex, chip) => document(chip).text()).get()),
      errors: [],
      href,
      name,
      parameterSizes: uniqueTexts(row.find('[x-test-size]').map((_chipIndex, chip) => document(chip).text()).get()),
...
export function parseModelTags(modelName: string, html: string): OllamaTag[] {
  ...
  document('input.command').each((_inputIndex, element) => {
    ...
    const metricColumns = grid.children('p').map((_columnIndex, column) => cleanText(document(column).text())).get()
    const inputTypes = parseInputTypes(grid.children('div').last().text())
    const metadataText = cleanText(desktopContainer.find('.font-mono').parent().text())
    const metadataParts = metadataText.split('·').map((part) => cleanText(part))
    const digest = (metadataParts[0] ?? '').match(/[a-f0-9]{12}/i)?.[0]?.toLowerCase() ?? null
    ...
    tags.push({
      ...
      digest,
      href: grid.find('a[href^="/library/"]').first().attr('href') ?? `/library/${tagName}`,
      inputTypes,
      name: tagName,
      parameterB: parseParameterEstimate(tagName),
      sizeBytes: parseByteSizeToBytes(sizeLabel),
      sizeLabel,
      tag: tagName.startsWith(`${modelName}:`) ? tagName.slice(modelName.length + 1) : tagName.split(':').at(-1) ?? tagName,
      updatedLabel,
    })
  })
...
export async function fetchOllamaCatalogue(options: { refresh?: boolean } = {}): Promise<OllamaCataloguePayload> {
  const now = Date.now()
  ...
  if (!options.refresh && cachedCatalogue && cachedCatalogue.expiresAt > now) {
    return {
      ...
      cache: {
        hit: true,
        ttlSeconds: Math.max(0, Math.round((cachedCatalogue.expiresAt - now) / 1_000)),
      },
    }
  }
  ...
  const models = await mapWithConcurrency(libraryModels, DEFAULT_TAG_FETCH_CONCURRENCY, async (model) => {
    try {
      const tagsHtml = await fetchOllamaHtml(`${OLLAMA_LIBRARY_URL}/${encodeURIComponent(model.slug)}/tags`)
      ...
      return {
        ...model,
        errors: [`Failed to fetch tags for ${model.name}: ${error instanceof Error ? error.message : String(error)}`],
        tags: [],
      }
    }
  })
...
function inferParametersFromDuplicateDigests(tags: OllamaTag[]): OllamaTag[] {
  ...
  for (const tag of tags) {
    if (tag.digest && tag.parameterB !== null) {
      ...
      parametersByDigest.set(tag.digest, Math.max(existingParameter ?? 0, tag.parameterB))
    }
  }
