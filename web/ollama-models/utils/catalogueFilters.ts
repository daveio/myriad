  ...
  return rows.filter((row) => {
    ...
    if (
      ...
      !requiredCapabilities.every((capability) =>
        row.capabilities.map(normaliseSearchText).includes(capability),
      )
    ) {
      return false;
    }
    ...
    if (
      ...
      !requiredInputTypes.every((inputType) =>
        row.inputTypes.map(normaliseSearchText).includes(inputType),
      )
    ) {
      return false;
    }
...
export function sortCatalogueRows(
  ...
  return [...rows].sort((leftRow, rightRow) => {
    const primaryComparison = compareNullable(
      sortValue(leftRow, sort.key),
      sortValue(rightRow, sort.key),
    );
    ...
    const secondarySizeComparison = compareNullable(
      leftRow.sizeBytes,
      rightRow.sizeBytes,
    );
...
export function summariseRows(rows: CatalogueTagRow[]): CatalogueSummary {
  ...
  return {
    ...
    totalPullCount: [...modelPullCounts.values()].reduce(
      (totalPullCount, pullCount) => totalPullCount + pullCount,
      0,
    ),
  };
}
...
export function collectInputTypes(rows: CatalogueTagRow[]): string[] {
  return [...new Set(rows.flatMap((row) => row.inputTypes))].sort(
    (leftValue, rightValue) => leftValue.localeCompare(rightValue),
  );
}
...
function passesMinimum(
  ...
  return (
    requiredValue === null ||
    (actualValue !== null && actualValue >= requiredValue)
  );
}
